 ...
 **Remark:** LAST(\<expression>, \<default>) would be equivalent of LAG(\<expression>, 0, \<default>) and is introduced for convenience and better readability. Note that LAG(\<expression>, 0, \<default>) is actually not a valid construct since LAG takes an offset greater or equal to 1.
