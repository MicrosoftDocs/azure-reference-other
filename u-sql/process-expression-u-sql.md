...
A processor operates on a row at a time and produces zero or one row. If the processing should produce multiple rows per input, an [Applier](../USQL/u-sql-using-apply-with-an-applier-udo.md) should be used instead.  
...
<a></a>     <a href="u-sql-identifiers.md">Quoted_or_Unquoted_Identifier</a> <a href="built-in-u-sql-types.md">Built_in_Type</a>.
    ...
    Each column has an identifier that can be either a [quoted or unquoted identifier](../USQL/u-sql-identifiers.md). A column is typed with one of the U-SQL types that the processor supports.  
