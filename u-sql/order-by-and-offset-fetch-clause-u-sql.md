...
U-SQL provides the optional `ORDER BY` clause to order a rowset generated by a query expression such as a [`SELECT`](../u-sql/select-expression-u-sql.md) expression, set expression or function invocation, and as an optional clause on the [`OUTPUT`](../u-sql/output-statement-u-sql.md) statement.
...
To output with a specific order, use `ORDER BY` with the [`OUTPUT`](../u-sql/output-statement-u-sql.md) statement which has no such restrictions and makes the `OFFSET/FETCH` clause optional. For more information on the [`OUTPUT`](../u-sql/output-statement-u-sql.md) statement's specific ordering and examples please refer to [Output Statement (U-SQL)](../u-sql/output-statement-u-sql.md). Note that the `ORDER BY` and `OFFSET/FETCH` semantics also apply to the [`OUTPUT`](../u-sql/output-statement-u-sql.md) statement’s `ORDER BY` clause.  
