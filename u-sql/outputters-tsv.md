...
The Tsv() outputter disallows the `delimiter` parameter and defaults the field delimiter to '\t' (tab).  All other parameters are the same.  See [Outputter Parameters (U-SQL)](../u-sql/outputter-parameters-u-sql.md) for supported parameters and their defaults values.
