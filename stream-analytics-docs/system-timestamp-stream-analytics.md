...
Here, we describe how Azure Stream Analytics assigns timestamps to events.  
...
```  
 ...
 If a TIMESTAMP BY clause isn't specified for a given input, arrival time of the event is used as a timestamp. For example Enqueued time of the event is used when input is Azure Event Hubs.  
...
```  
 ...
 Projections don't alter the timestamp of the event. The timestamp of the result is the same as the timestamp of the input.  
...
```  
 ...
 Filters don't alter the timestamp of the event. The timestamp of the result is the same as the timestamp of the input.  
...
```  
 ...
 The timestamp of the result of the aggregate is the end of the time window to which this result corresponds. See [Windowing](windowing-azure-stream-analytics.md) articles describing different window types in Azure Stream Analytics.  
...
```  
 ...
 The events that represent a match (has both input1 and input2) is timestamped by the latest of the timestamps of the matching inputs (as is with the INNER JOIN above).  
  
 The events that represent nonmatches (input2 is NULL) are timestamped by the latest logical time at which a matching input2 event could have occurred. For instance, in the example, it's input1’s timestamp + 10 minutes.  
