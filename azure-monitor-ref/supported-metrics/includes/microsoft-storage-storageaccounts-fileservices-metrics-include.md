...
|Transaction|**Metadata Warning Metric**<br><br>Successful request when a client runs high metadata IOPS, which may cause throttling later if the IOPS remain high or increase. |`SuccessWithMetadataWarning` |Count |Sum |`ResponseType`|PT1M |Yes|
|Transaction|**Metadata Throttling Metric**<br><br>Successful request when a client gets throttled due to high metadata IOPS in the first attempt(s) but succeeds after retries. |`SuccessWithMetadataThrottling` |Count |Sum |`ResponseType`|PT1M |Yes|
