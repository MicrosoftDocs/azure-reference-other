...
```query
ACSCallClientOperations
| where ResultType !in ('Succeeded', 'Success', 'ExpectedClientError')
| where OperationName in ('Join', 'EnterCall')
| where OperationPayload.CallDirection == 'Outgoing'
| summarize Count=count() by tostring(OperationPayload.FailureReason)
