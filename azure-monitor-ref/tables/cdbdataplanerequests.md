...
# CDBDataPlaneRequests
 ...
 The DataPlaneRequests table captures every data plane operation for the Azure Cosmos DB account. Data Plane requests are operations executed to create, update, delete or retrieve data within the account.
...
| AuthTokenType | string | The authorization type (System Read/Write key) for this request by the Azure Cosmos DB Gateway service when running in Gateway mode or using the REST API. |
| ClientIpAddress | string | The IP address of the client VM issuing the request. |
| CollectionName | string | The Azure Cosmos DB container against which the request was issued. |
| ConnectionMode | string | The connection mode used by the client issuing the request – (Direct or Gateway mode). |
| DatabaseName | string | The Azure Cosmos DB database against which the request was issued. |
...
| PartitionId | string | The physical partition ID for the Azure Cosmos DB container against which the request was issued. |
...
| RequestResourceId | string | The ID of the specific Azure Cosmos DB resource within the account against which the data plane request was executed. |
| RequestResourceType | string | The Azure Cosmos DB resource type within the account against which the data plane request was executed, can be one of Database, Collection, Document, Attachment, User, Permission, StoredProcedure, Trigger, UserDefinedFunction, Offer. |
...
| UserAgent | string | The name of the user specified user agent suffix (as specified when initializing the Azure Cosmos DB client) when running in Direct mode. |
