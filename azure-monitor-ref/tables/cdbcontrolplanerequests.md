...
| --- | --- | --- |
| AccountName | string | The name of the Azure Cosmos DB account. |
| AcledSubnets | string | The ACL’d subnets for the account. |
| ActivityId | string | The Activity ID of the operation. |
| ApiKind | string | The API kind for the account (SQL, Graph, MongoDB, Cassandra, Table) that is specified during account creation. |
...
| BackupIntervalInMinutes | real | The time (in minutes) between consecutive backup snapshots for the Azure Cosmos DB account. |
| BackupRetentionIntervalInHours | real | The duration of time (in hours) for which backup snapshots are retained for the Azure Cosmos DB account. |
...
| DefaultConsistencyLevel | string | The default consistency level for the Azure Cosmos DB account. |
| DurationMs | real | The time taken (in milliseconds) for this control plane operation to complete. |
| EnableAutomaticFailover | bool | Boolean flag to enable automatic failover for the Azure Cosmos DB account. |
| EnableCassandraRequestsTrace | bool | Boolean flag indicating if diagnostic logs are enabled for all Azure Cosmos DB for Apache Cassandra operations. |
...
| EnableMongoRequestsTrace | bool | Boolean flag indicating if diagnostic logs are enabled for MongoDB operations. |
| EnablePrivateEndpointConnection | bool | Boolean flag to enable private endpoints for the Azure Cosmos DB account. |
...
| IpRangeFilter | string | The IP range filter specified as part of the VNet rules for the Azure Cosmos DB account. |
| MaxStalenessIntervalInSeconds | real | The maximum staleness value (in seconds) for the Azure Cosmos DB account when using the Bounded Staleness consistency setting. |
| MaxStalenessPrefix | string | The max staleness prefix for the Azure Cosmos DB account when using the Bounded Staleness consistency setting. |
| MultipleWriteLocations | bool | Boolean flag indicating if the Azure Cosmos DB account is a multi-master account. |
| NewWriteRegion | string | The new write region for the Azure Cosmos DB account (after a user-initiated failover operation is executed). |
...
| PrivateEndpointConnections | string | The list of private endpoints for the Azure Cosmos DB account (in each region). |
...
| ResourceUri | string | The URI of the Azure Cosmos DB resource (e.g. Database, Container) against which the control plane operation was execution. |
...
| TenantId | string |  |
| TimeGenerated | datetime | Timestamp (in UTC) when this Control Plane operation was executed against the Azure Cosmos DB account. |
