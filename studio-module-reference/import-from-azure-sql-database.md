...
5.  Run the experiment. As [Import Data](import-data.md) loads the data into Studio, some implicit type conversion might also be performed, depending on the data types used in the source database. 
...
#### What happens if the database is in a  different geographical region. Can [Import Data](import-data.md) still access the database? Where will the data be stored?
