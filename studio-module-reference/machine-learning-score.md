...
This section lists the modules provided in Azure Machine Learning for Studio for *scoring*. You can find the
...
Scoring might be a new term to you, but is widely used in machine learning to mean the process of generating new values given a model and some new input. The generic term "score" is used, rather than "prediction", becuase the scoring process can generate so many different types of values:
...
> [!NOTE] 
> You might have also heard the word *score* used to mean a weight or value assigned as a result of data analysis. However, in Studio, _scoring_ usually denotes the process of generating predicted values from new data.
...
The new data that you provide as input generaly needs to have the same columns that were used to train the model, minus of course the label, or outcome column. 
...
Azure Machine Learning Studio provides many different scoring modules. You select one depending on the type of model you are using, or the type of scoring task you are performing: 
...
+ [Predict Book Reviews](https://gallery.cortanaintelligence.com/Experiment/Predict-Book-Reviews-1): Scoring on text data, using a logistic regression model.
