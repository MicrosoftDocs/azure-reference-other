...
This module is particularly useful when you need to separate data into training and testing sets. You can customize the way that data is divided as well. Some options support randomization of data; others are tailored for a ceretain data type or model type
 
## How to Use Split Data

1. First, for **Splitting mode**, choose one of the following options, depending on the type of data you have, and how you want to divide it. 

    > [!TIP]
    Read all options before choosing the splitting mode, because all other options change dynamically depending on that choice!

+ **Split Rows**: Use this option if you just want to divide the data into two parts. 
...
- **Relative Expression Split**:  Use this option whenever you want to apply a condition to a number column. 
    ...
    The number could be a date/time field, a column containing age or dollar amounts, or even a percentage. For example, you might want to divide your data set depending on the cost of the items, group people by age ranges, or separate data by a calendar date.
  
2. After you choose the **splitting mode**, new options are displayed, to help you customize the division of data. 

    Click the topics below for details of each type and examples of how to configure the properties. 
...
-   You can create only two sets at a time, and those sets must be exclusive. Therefore, if you have a complex split with multiple conditions and outputs (like a CASE statement), you must create multiple splits (like nested IF statements). In other words,  if you need to divide a dataset into many distinct sets, you can chain together multiple [Split Data](split-data.md) modules.
  
-   The [Split Data](split-data.md) module doesn't delete data or remove it from the dataset; it just divides the data as specified among the first and second outputs of the module. 
...
##  <a name="bkmk_Examples"></a> Examples  
 ...
 You can see examples of how the [Split Data](split-data.md) module is used by exploring these sample experiments in the [Cortana Intelligence Gallery](https://gallery.cortanaintelligence.com/):  
  
-   In the sample, [Cross Validation for Binary Classification: Adult Dataset](http://go.microsoft.com/fwlink/?LinkId=525734), a 20% sampling rate is applied to create a smaller randomly sampled dataset. (The original census dataset had over 30,000 rows; the training dataset has around 6500). The dataset is cleaned for missing values and then passed to five different models for training and cross-validation.  
...
-  The input dataset must contains at least two rows.  
