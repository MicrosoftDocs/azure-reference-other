...
## How to Configure a Quantile Regression Model  
 The properties used for configuring a Fast Forest Quantile Regression model differ considerably dependng on whether you are providing a fixed set of parameters, or setting up a parameter sweep.  
...
###  <a name="bkmk_FixedParameters"></a> To create a quantile regression model using fixed parameters  
  
1.  Add the **Fast Forest Quantile Regression** module to the experiment.  
  
2.  If you know how you want to configure the model, you can provide a specific set of values as arguments. To do this, sett the **Create trainer mode** option to **Single Parameter**.  
  
     In this case, you should train the model using [Train Model](train-model.md) and not [Tune Model Hyperparameters](tune-model-hyperparameters.md).  
...
###  <a name="bkmk_ParameterSweep"></a> To create a quantile regression model using a parameter sweep  
...
2.  Set the **Create trainer mode** option to **Parameter Range**.  
     ...
     After choosing a parameter sweep,  for most properties, you can set values in these ways:  
  
    -   Type a single value to be used across all iterations. F  
  
    -   Type a comma-separated list of discrete values to use.  
  
    -   Select **Range Builder** and define a range of values.  
  
     You can use different methods on each property; for example, you might fix the number of trees, but vary other values that control the way each tree is built. When the model is trained, the [Tune Model Hyperparameters](tune-model-hyperparameters.md) module will iterate over various combinations of the values to build the best model.  
  
3.  For **Maximum number of leaves per tree**, type the total number of leaves, or terminal nodes, to allow in each tree.  
  
4.  For **Number of trees constructed**, type the number of iterations to perform when constructing the ensemble. By creating more trees, you can potentially get better coverage, but training time will increase.  
...
6.  In **Range for bagging fraction**, type the fraction of samples to use when building each group of quantiles. Samples are chosen randomly, with replacement.  
     ...
     Each fraction should be a number between 0 and 1; separate multiple fractions by commas.  
...
8.  In **Range for split fraction**, specify the fraction of features to use in each group of quantiles. The features used are chosen randomly.  
...
10. In **Required quantile values**, type a comma-separated list of the quantiles on which you want the model to train and create predictions.  
...
13. Connect a training dataset, select a single label column, and connect the [Tune Model Hyperparameters](tune-model-hyperparameters.md) module.  
  
    > [!NOTE]
    >  Do not use [Train Model](train-model.md). If you configure a parameter range but train using [Train Model](train-model.md), it will use only the first value in the parameter range list.  
  
14. Run the experiment.  
  
15. When the model has been trained, you can right-click the output of the [Tune Model Hyperparameters](tune-model-hyperparameters.md) module and select **Visualize** to see the final hyperparameters of the optimized model.  
  
## Examples  
 For examples of how to use this module, see these sample experiments in the [Cortana Intelligence Gallery](https://gallery.cortanaintelligence.com):  
