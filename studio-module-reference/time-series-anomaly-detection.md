...
<a name="ModuleOverview"></a>   
## Module Overview  
 This module learns the normal operating characteristics of a time series, and then uses that information to detect deviations from the normal pattern. The module supports detection of changes in the overall trend over a time series, as well as detecting changes in the range of values for consistently positive and negative trends.  
...
### About Anomaly Detection Methods  
...
### Anomaly Detection Examples  
 This module supports detecting two types of anomalous patterns in time series:  
...
10. When training is complete, the module outputs a time series that is the same length as the input time series; however, two columns are added to indicate values that are potentially anomalous.  
...
### Detecting Level Changes  
 To assess the impact of different settings, we used an example dataset consisting of 8870 data points, with three level changes. Based on this data, we created two models, using the following parameters. We then applied a different value for **Alert Threshold**.  
...
-   **Length of Strangeness Values** = 50  
 ...
 The model was trained on the incoming data and the results of prediction plotted as shown in the following images for Model 1 and Model 2. In these graphs, the blue lines represent data values and the red lines represent alerts raised for an anomaly.  
...
### Detecting Positive Slope Changes  
