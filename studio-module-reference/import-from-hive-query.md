...
3.  In the **Hive database query** text box, specify the data you want to read by using HiveQL.  
     ...
     HiveQL is a SQL-like query language that can also be used to aggregate data and perform data filtering before you add the data to Machine Learning Studio. However, the Hive query must return the data in a tabular format. For example, this statement is a valid Hive query:  
