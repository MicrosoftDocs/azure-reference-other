...
This article describes how to use the **Export to Azure Blo Storage** option, in the [Export Data](export-data.md) module in Azure Machine Learning Studio. 

This options is useful when you want to to export data from a machine learning experiment to Azure blob storage. For example, you might want to share machine learning data outputs with other applications, or store intermediate data or cleaned datasets for use in other experiments. 
...
5. **Path to container, directory, or blob**: Type the name of the blob where the exported data will be stored. For example, to save the results of your experiment to a new blob named **results01.csv** in the container **predictions** in an account named **mymldata**, the full URL for the blob would be `http://mymldata.blob.core.windows.net/predictions/results01.csv` 
...
6.  If you specify the name of a blob that does not already exist, Azure creates the blob for you.  
...
8.  **Use cached results**: Select this option if you want to avoid rewriting the results to the blob file each time you run the experiment. If there are no other changes to module parameters, the experiment writes the results only the first time the module is run, or when there are changes to the data.
