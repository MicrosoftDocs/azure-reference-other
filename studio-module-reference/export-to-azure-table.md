...
4.  Specify which columns to save to the table store, and which columns to use in defining the table schema, by using the column properties.  
    ...
    > [!NOTE]
    >  You must use different columns for **RowKey** and **PartitionKey**. Make sure that any column you select for the RowKey or PartitionKey is also included in the list of destination columns, or you will get an error.  
...
6. **Azure table write mode**: Indicate how you want the [Export Data](export-data.md) to behave when data already exists in the Azure table.  
...
7.  **Use cached results**: Indicate whether you want the data to be refreshed each time the experiment is run. 
